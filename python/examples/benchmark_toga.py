...
def benchmark_initialization():
    ...
    results = benchmark_function(
        lambda: initialize_toga_personality(),
        iterations=10000
    )
    ...
    results = benchmark_function(
        lambda: initialize_toga_personality(custom_traits),
        iterations=10000
    )
...
def benchmark_input_framing():
    ...
    # Normal message
    results = benchmark_function(
        lambda: toga.frame_input("Normal message without triggers"),
        iterations=10000
    )
    ...
    results = benchmark_function(
        lambda: toga.frame_input("This is so cute and adorable!"),
        iterations=10000
    )
...
def benchmark_emotional_state():
    ...
    # State update
    results = benchmark_function(
        lambda: toga.update_emotional_state("excited", 0.9, 2.0, "test"),
        iterations=10000
    )
...
def benchmark_commentary():
    ...
    for context in contexts:
        results = benchmark_function(
            lambda: toga.add_commentary("Test content", context=context),
            iterations=10000
        )
...
def benchmark_full_interaction():
    ...
    # Normal interaction
    results = benchmark_function(
        lambda: toga.process_interaction("Hello, how are you?"),
        iterations=5000
    )
    ...
    results = benchmark_function(
        lambda: toga2.process_interaction("This is so cute!"),
        iterations=5000
    )
...
def benchmark_serialization():
    ...
    data = toga.to_dict()
    results = benchmark_function(
        lambda: TogaPersonality.from_dict(data),
        iterations=5000
    )
...
def benchmark_trait_evolution():
    ...
    results = benchmark_function(
        lambda: tensor.evolve_trait("cheerfulness", 0.01),
        iterations=10000
    )
    ...
    results = benchmark_function(
        lambda: tensor.evolve_trait("no_actual_harm", -0.5),
        iterations=10000
    )
...
def benchmark_inheritance():
    ...
    # 70% inheritance
    results = benchmark_function(
        lambda: parent.inherit(inheritance_factor=0.7),
        iterations=5000
    )
    ...
    # 50% inheritance
    results = benchmark_function(
        lambda: parent.inherit(inheritance_factor=0.5),
        iterations=5000
    )
